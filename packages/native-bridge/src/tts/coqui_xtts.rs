//! Coqui XTTS v2 voice cloning engine — communicates with a persistent Python HTTP server.
//!
//! The Coqui XTTS server runs on localhost:21745 and handles model loading
//! and voice cloning synthesis using the XTTS v2 model.
//!
//! Start the server with:
//!   cd packages/native-bridge && python3.11 coqui_xtts_server.py
...
const DEFAULT_URL: &str = "http://127.0.0.1:21745";

pub struct CoquiXttsSynthesizer {
    base_url: String,
}
...
impl TtsSynthesizer for CoquiXttsSynthesizer {
    fn synthesize(
        &self,
        text: &str,
        request_id: &str,
        ...
        let voice_code = options
            .voice_id
            .as_deref()
            .and_then(|id| id.strip_prefix("coqui-xtts:"))
            .unwrap_or("default");
        ...
        let mut resp = ureq::post(&url)
            ...
            .map_err(|e| {
                TtsError::SynthesisFailed(format!(
                    "Coqui XTTS server request failed (is coqui_xtts_server.py running?): {e}"
                ))
            })?;
        ...
        if status != 200 {
            ...
            return Err(TtsError::SynthesisFailed(format!(
                "Coqui XTTS server returned status {status}"
            )));
        }
        ...
        if pcm_bytes.len() < 4 {
            return Err(TtsError::SynthesisFailed(
                "Coqui XTTS produced no audio".into(),
            ));
        }
        ...
        println!(
            "  [coqui-xtts] Produced {} samples ({:.1}s) @ {}Hz",
            samples.len(),
    ...
    fn list_voices(&self) -> Result<Vec<VoiceDescriptor>, TtsError> {
        ...
        let mut resp = ureq::get(&url).call().map_err(|e| {
            TtsError::NotAvailable(format!("Coqui XTTS server not reachable: {e}"))
        })?;
        ...
        Ok(infos
            ...
            .map(|v| VoiceDescriptor {
                id: format!("coqui-xtts:{}", v.id),
                name: format!("XTTS Clone: {}", v.name),
                language: v.language,
                gender: v.gender,
                engine: "coqui-xtts".to_string(),
                quality: Some("neural-clone".to_string()),
                description: Some(format!("Coqui XTTS v2 voice clone — {}", v.name)),
