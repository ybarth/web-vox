//! Qwen3-TTS Base voice cloning engine — communicates with a persistent Python HTTP server.
//!
//! The Qwen3-TTS Clone server runs on localhost:21746 and handles model loading
//! and voice cloning synthesis using the Qwen3-TTS Base model.
//!
...
const DEFAULT_URL: &str = "http://127.0.0.1:21746";

pub struct QwenCloneSynthesizer {
    base_url: String,
}
...
impl TtsSynthesizer for QwenCloneSynthesizer {
    fn synthesize(
        &self,
        text: &str,
        request_id: &str,
        ...
        let voice_code = options
            .voice_id
            .as_deref()
            .and_then(|id| id.strip_prefix("qwen-clone:"))
            .unwrap_or("default");
        ...
        let mut resp = ureq::post(&url)
            ...
            .map_err(|e| {
                TtsError::SynthesisFailed(format!(
                    "Qwen3-TTS Clone server request failed (is qwen_tts_clone_server.py running?): {e}"
                ))
            })?;
        ...
        if status != 200 {
            ...
            return Err(TtsError::SynthesisFailed(format!(
                "Qwen3-TTS Clone server returned status {status}"
            )));
        }
        ...
        if pcm_bytes.len() < 4 {
            return Err(TtsError::SynthesisFailed(
                "Qwen3-TTS Clone produced no audio".into(),
            ));
        }
        ...
        println!(
            "  [qwen-clone] Produced {} samples ({:.1}s) @ {}Hz",
            samples.len(),
    ...
    fn list_voices(&self) -> Result<Vec<VoiceDescriptor>, TtsError> {
        ...
        let mut resp = ureq::get(&url).call().map_err(|e| {
            TtsError::NotAvailable(format!("Qwen3-TTS Clone server not reachable: {e}"))
        })?;
        ...
        Ok(infos
            ...
            .map(|v| VoiceDescriptor {
                id: format!("qwen-clone:{}", v.id),
                name: format!("Qwen Clone: {}", v.name),
                language: v.language,
                gender: v.gender,
                engine: "qwen-clone".to_string(),
                quality: Some("neural-clone".to_string()),
                description: Some(format!("Qwen3-TTS voice clone — {}", v.name)),
