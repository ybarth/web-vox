//! Qwen3-TTS engine — communicates with a persistent Python HTTP server.
//!
//! The Qwen3-TTS server runs on localhost:21744 and handles model loading
//! and synthesis using Alibaba's Qwen3-TTS models.
//!
...
const DEFAULT_URL: &str = "http://127.0.0.1:21744";
...
impl QwenSynthesizer {
    ...
    /// Check if the Qwen3-TTS server is running.
...
impl TtsSynthesizer for QwenSynthesizer {
    fn synthesize(
        &self,
        text: &str,
        request_id: &str,
        ...
        let voice_code = options
            .voice_id
            .as_deref()
            .and_then(|id| id.strip_prefix("qwen:"))
            .unwrap_or("Ryan");
        ...
        let mut resp = ureq::post(&url)
            ...
            .map_err(|e| {
                TtsError::SynthesisFailed(format!(
                    "Qwen3-TTS server request failed (is qwen_tts_server.py running?): {e}"
                ))
            })?;
        ...
        if status != 200 {
            ...
            return Err(TtsError::SynthesisFailed(format!(
                "Qwen3-TTS server returned status {status}"
            )));
        }
        ...
        if pcm_bytes.len() < 4 {
            return Err(TtsError::SynthesisFailed(
                "Qwen3-TTS produced no audio".into(),
            ));
        }
        ...
        println!(
            "  [qwen-tts] Produced {} samples ({:.1}s) @ {}Hz",
            samples.len(),
    ...
    fn list_voices(&self) -> Result<Vec<VoiceDescriptor>, TtsError> {
        ...
        let mut resp = ureq::get(&url).call().map_err(|e| {
            TtsError::NotAvailable(format!("Qwen3-TTS server not reachable: {e}"))
        })?;
        ...
        Ok(infos
            ...
            .map(|v| VoiceDescriptor {
                id: format!("qwen:{}", v.id),
                name: format!("Qwen: {}", v.name),
                language: v.language,
                gender: v.gender,
                engine: "qwen".to_string(),
                quality: Some("neural".to_string()),
                description: Some(format!("Qwen3-TTS — {}", v.name)),
